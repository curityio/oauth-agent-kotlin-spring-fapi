...
@Service
class CookieEncrypter(private val config: OAuthAgentConfiguration, private val cookieName: CookieName)
{
        ...
        return withContext(Dispatchers.Default) {
            kotlin.run {
                ...
                if (allBytes.size < minSize) {
                    throw InvalidCookieException("The received cookie has an invalid length")
                }

                val version = allBytes[0].toInt()
                if (version != CURRENT_VERSION) {
                    throw InvalidCookieException("The received cookie has invalid format")
                }
